...
int main(int argc, char *argv[]) {
    ...
    hipHostMalloc( (void **) &facesPerPoint, sizeof(int) * totalFacesPerPoint);
#else
	...
	#pragma omp parallel default(none)  shared(stdout, logSqrt, nDim, nPoints, nFaces, nVertsPerFace, numThreads, preproc_times, kernel_times, v_points, v_points_faces, offsets, offsets_faces, faces, coords, nFacesPerPoint, facesPerPoint, flowmap, t_eval, startIndices, totalFacesPerPoint)  
	{
        ...
        hipMalloc( &d_startIndices,   sizeof(int) * nPoints );
		hipMalloc( &d_writeIndices,   sizeof(int) * nPoints );
		hipMalloc( &d_facesPerPoint,  sizeof(int) * totalFacesPerPoint );
        ...
        hipMemcpy( d_startIndices,   startIndices,   sizeof(int) * nPoints,  hipMemcpyHostToDevice );
		hipMemset( d_writeIndices,   0,              sizeof(int) * nPoints );
		...
		hipLaunchKernelGGL(create_facesPerPoint_vector_opt, grid_numCoords, block, 0, hipStreamDefault, nFaces, nVertsPerFace, d_faces, d_startIndices, d_writeIndices, d_facesPerPoint);
